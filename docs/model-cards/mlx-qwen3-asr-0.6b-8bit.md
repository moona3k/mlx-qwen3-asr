...
8-bit (group size 64) MLX quantization of [Qwen/Qwen3-ASR-0.6B](https://huggingface.co/Qwen/Qwen3-ASR-0.6B)
...
HTTP server (`mlx-qwen3-asr serve --model moona3k/mlx-qwen3-asr-0.6b-8bit`) work unchanged.
...
- Download size: 802M (fp16 source: 1.8 GB).
...
python scripts/convert.py --model Qwen/Qwen3-ASR-0.6B --quantize 8 --group-size 64 \
  --dtype float16 --output-dir Qwen3-ASR-0.6B-8bit-g64
